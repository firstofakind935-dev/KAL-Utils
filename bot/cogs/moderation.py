...
class Moderation(commands.Cog):
    ...
    async def promote(self, ctx: commands.Context, member: discord.Member, *, role: discord.Role):
        """Promote a member by assigning them a role."""
        if role >= ctx.guild.me.top_role:
            return await ctx.send("I can't assign a role that's at or above my highest role.")
    ...
    async def demote(self, ctx: commands.Context, member: discord.Member, *, role: discord.Role):
        """Demote a member by removing a role from them."""
        if role >= ctx.guild.me.top_role:
            return await ctx.send("I can't remove a role that's at or above my highest role.")
        if role not in member.roles:
            return await ctx.send(f"{member.mention} doesn't have **{role.name}**.")
    ...
    async def roles(self, ctx: commands.Context):
        """List all assignable server roles."""
        assignable = [
